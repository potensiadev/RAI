# PRD: Position-Candidate Hybrid Semantic Matching v1.0
...
| **주기** | 매 시간 (경량) / 매일 (전체) |
| **작업 내용** | 1. 미검증 스킬 쌍 LLM 분석<br>2. 스킬 카테고리 분류<br>3. 오래된 매핑 재검증<br>4. 미사용 데이터 정리 |
| **수용 기준** | 1. 일일 처리량 > 1000쌍<br>2. 시스템 부하 < 10%<br>3. 비용 < $20/일 |
...
| 항목 | 현재 예상 | 1년 후 목표 |
|-----|----------|-----------|
| 스킬 종류 | 5,000개 | 20,000개 |
| 시맨틱 캐시 | 50,000쌍 | 500,000쌍 |
| 후보자 수 | 50,000명 | 500,000명 |
| JD 수 | 5,000개 | 50,000개 |
| 동시 접속 | 100명 | 1,000명 |
...
| 항목 | 단가 | 월 예상 비용 |
|-----|-----|------------|
| OpenAI 임베딩 | $0.0001/스킬 | $10 (10만 스킬) |
| LLM 분석 (즉시) | $0.01/스킬 | $100 (1만 신규 스킬) |
| LLM 배치 학습 | $10/일 | $300 |
| **총계** | | **~$410/월** |
...
다음 기술/스킬을 분석해주세요: "{skill_name}"

JSON으로 응답해주세요:
{{
  "canonical_name": "정규화된 스킬명 (영문 기준)",
  "korean_names": ["한글 표기 변형들"],
  "english_names": ["영문 표기 변형들"],
  "related_skills": [
    {{
      "name": "관련 스킬명",
  ...
  "parent_category": "상위 카테고리 (Frontend, Backend, DevOps 등)",
  "skill_type": "tool|framework|language|practice|certification|soft_skill",
  "description": "이 스킬에 대한 간단한 설명 (1-2문장)"
}}
...
| **비용 초과** | 운영 비용 증가 | Low | 일일 처리량 제한, 캐시 적극 활용 |
| **콜드 스타트** | 초기 정확도 낮음 | High | 시드 데이터 준비, 점진적 롤아웃 |
