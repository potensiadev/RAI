    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="p-5 rounded-xl bg-white border border-gray-100 shadow-sm">
          ...
          <p className="text-3xl font-bold text-gray-900 mt-4">{data.totalCandidates}</p>
        ...
        <div className="p-5 rounded-xl bg-white border border-gray-100 shadow-sm">
          ...
          <p className="text-3xl font-bold text-gray-900 mt-4">{data.highConfidenceCount}</p>
          <p className="text-sm text-gray-500">높은 신뢰도 (95%+)</p>
        </div>
        ...
        <div className="p-5 rounded-xl bg-white border border-gray-100 shadow-sm">
          ...
          <p className="text-3xl font-bold text-gray-900 mt-4">{data.totalExports}</p>
          <p className="text-sm text-gray-500">블라인드 내보내기</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-xl bg-white border border-gray-100 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">월별 등록 추이</h3>
        ...
        {/* Experience Distribution */}
        <div className="p-6 rounded-xl bg-white border border-gray-100 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">경력 분포</h3>
          <div className="space-y-3">
            {data.expDistribution.map((item, index) => (
              <div key={index} className="flex items-center gap-3">
                ...
                <span className="w-8 text-sm text-gray-900 text-right">{item.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Top Skills */}
        <div className="p-6 rounded-xl bg-white border border-gray-100 shadow-sm col-span-2">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">상위 스킬</h3>
          <div className="grid grid-cols-2 gap-x-8 gap-y-3">
            {data.skillDistribution.map((item, index) => (
              <div key={index} className="flex items-center gap-3">
                <span className="w-4 text-xs text-gray-400">{index + 1}</span>
                <span className="w-32 text-sm text-gray-600 truncate">{item.skill}</span>
